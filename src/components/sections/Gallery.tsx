...
export default function Gallery() {
  ...
  return (
    <>
      <section id="gallery" className="py-24 md:py-32 bg-bg-secondary w-full">
        ...
        <div className="container mx-auto px-6 md:px-12 w-full">
          <div 
            ...
            className="flex flex-col md:flex-row md:overflow-x-auto gap-6 md:gap-8 pb-8 relative snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
          >
            {galleryImages.map((src, idx) => (
              <div 
                key={idx} 
                className={`gallery-card w-full md:w-[calc(33.333%-1rem)] lg:w-[calc(33.333%-1.333rem)] flex-shrink-0 aspect-[3/4] cursor-pointer group sticky top-[15vh] md:static snap-center shadow-xl md:shadow-none ${idx >= 3 ? 'hidden md:block' : ''}`}
                onClick={() => setLightboxIndex(idx)}
              >
                <div className="relative w-full h-full overflow-hidden">
                  <div className="absolute inset-0 bg-marble z-0"></div>
                  <Image
                    src={src}
